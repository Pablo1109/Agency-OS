...
import { updateRecurringFinanceAction } from "@/lib/actions";
...
export default async function RecurrencesPage() {
  ...
  return (
    <>
      <PageHeader
        title="Recorrencias"
        description="Edite despesas fixas que entram na previsao do financeiro."
      />
        ...
        {recurrences.length === 0 ? (
          <Card>
            <CardContent className="p-6 text-sm text-muted-foreground">
              Nenhuma despesa recorrente cadastrada ainda. No financeiro, crie uma despesa a vista e marque como recorrente fixa.
            </CardContent>
          </Card>
        ) : null}
          ...
          <Card key={entry.id}>
            <CardHeader className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
              ...
              <div className="flex items-center gap-2">
                <Badge className="border-slate-200 bg-slate-50 text-slate-700">Recorrente</Badge>
